...
impl Termbox {
  ...
  pub fn new () -> Result<Termbox> {
    unsafe {
      if _is_open.swap(true, Ordering::SeqCst) {
      ...
      match ::ffi::tb_init() {
        ...
        ::ffi::TB_EUNSUPPORTED_TERMINAL => {
          _is_open.store(false, Ordering::SeqCst);
...
fn from_ffi_event (ffi_event: ::ffi::tb_event) -> Option<Event> {
  match ffi_event.kind {
    ::ffi::TB_EVENT_KEY => {
      let keysym = KeySym {
        mods: if let Some(mods) = Mods::from_bits(ffi_event.mods) {mods} else { return None; },
        key: ffi_event.key,
        ch: if let Some(ch) = from_u32(ffi_event.ch) {ch} else { return None; },
      };
      return Some(Event::Key(keysym));
    },

    ::ffi::TB_EVENT_RESIZE => {
      return Some(Event::Resize(ffi_event.w, ffi_event.h));
    },
